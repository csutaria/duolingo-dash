...
> - Endpoint ⑨ (`pathSectioned`) is fetched once per `saveLanguageDetails` call and used to override the legacy `levels_finished` value when present.
> - Course-cycling syncs re-read endpoint ① around switch/fetch/restore steps and before/after active-course-dependent detail fetches. If the account's active course changes outside Dash's expected sequence, the sync aborts with `Active course changed outside this sync`, stops switching, and does not restore over the other actor's course. Clean course details are written from an in-memory draft; drift-tainted course details are skipped. The Redis/Valkey account lock is still the hard cross-server guard.

...
> - Course-cycling syncs now re-read endpoint ① around switch/fetch/restore steps to warn if the account's active course changes outside Dash's expected sequence. These checks improve restore observability; the Redis/Valkey account lock is still the hard cross-server guard.

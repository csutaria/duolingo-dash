...
| --------------------------------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 writer (single Node)                              | **Yes — primary mode.** | All polling timers, the JWT, the resolver cache, and the polling state live in the writer's `globalThis`. SQLite WAL with a single writer.                                                                                                                                                                                                                                                       |
...
| `isRunning` (single-flight mutex)                           | `src/lib/polling.ts` (via `globalThis`)        | No.                                                       | Process-local mutex doesn't gate cross-process syncs. Would need a DB-level advisory lock or a status row (`sync_in_progress = 1`) checked + atomically claimed before `fullSync` starts.                                                                                          |
| `mode` (`"baseline"` / `"fast"`)                            | `src/lib/polling.ts` (via `globalThis`)        | No (recomputed at startup; defaults to baseline).         | Move to a DB row so all writers agree on whether the quiet-detector is active. Without this, writer A in baseline + writer B in fast = double-coverage.                                                                                                                            |
| `fastLastObservedXp` / `fastConsecutiveIdleTicks`           | `src/lib/polling.ts`                           | No.                                                       | Same — the fast-mode bookkeeping has to be authoritative, not per-process.                                                                                                                                                                                                         |
...
5. **No leader-election.** If a follower is accidentally started
   ...
   only safeguard is operational: keep `DUOLINGO_READ_ONLY=1` set on
   the follower.
...
- Read-only mode side-effects: `docs/architecture.md` § Read-only mode (`DUOLINGO_READ_ONLY=1`).

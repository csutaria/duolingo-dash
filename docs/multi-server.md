...
| `isRunning` (single-flight mutex)                           | `src/lib/polling-state.ts` (via `globalThis`), claimed by `src/lib/sync-lock.ts` | No.                                                       | Gates all sync mutations inside the single writer process, including requests from multiple browsers. Process-local mutex doesn't gate cross-process syncs. Would need a DB-level advisory lock or a status row (`sync_in_progress = 1`) checked + atomically claimed before `fullSync` starts. |

...
| `courseOrderRecoveryTarget`                                  | `src/lib/polling.ts`                           | No.                                                       | Preserves the pre-cycle selector order after an automatic conflict so the same process can repair a half-cycled order after quiet/backoff. True multi-writer recovery would need this target in shared state, or the first writer's pre-conflict order can be lost on restart/another process. |

...
| `sync-cycle-order.test.ts`     | All-course sync course-selector order preservation, active-course drift conflicts, drift-only per-course detail atomicity, manual per-course restore success/failure reporting |

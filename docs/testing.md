...
| `polling.test.ts`              | Refresh cooldown enforcement, XP change detection, first-sync trigger, `advanceSyncState` reducer, automatic account-quiet jitter retry, final pre-retry XP/course recheck, gate-busy re-monitoring, nightly entering the same account-quiet gate, manual refresh external-lock/conflict skips, `msUntilNextLocalTime` (next 02:00 in **R**), HMR `globalThis` polling bucket |
| `sync-cycle-order.test.ts`     | All-course sync course-selector order preservation, active-course / XP drift conflicts, drift-only per-course detail atomicity, manual per-course restore success/failure reporting |

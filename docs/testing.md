...
| `polling.test.ts`              | Refresh cooldown enforcement, XP change detection, first-sync trigger, `advanceSyncState` reducer, fast-mode local/external gate-busy retry behavior, manual refresh external-lock skip without consuming cooldown, `msUntilNextLocalTime` (next 02:00 in **R**), HMR `globalThis` polling bucket |
...
| `src/app/api/sync/__tests__/route.test.ts` | POST `/api/sync`: `DUOLINGO_READ_ONLY` (`1`/`true`/`yes`) → 503 `{ error: "read-only" }` and never invokes `ensureClient`/`fullSync`/`manualRefresh`; normal mode dispatches `manualRefresh` vs `fullSync(_, cycleAll)` correctly; local/external busy force-sync returns skipped without calling `fullSync` |
| `src/app/api/sync-course/__tests__/route.test.ts` | POST `/api/sync-course`: 503 in read-only without touching the writer path; 400 on missing required params; local/external busy writer returns skipped/error shape without calling `syncCourseDetails` |
